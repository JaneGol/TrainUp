...
export default function MultiStepMorningDiaryForm() {
  ...
  // Check if any of the specified muscles are selected
  const hasSelectedMuscle = (...bodyParts: string[]): boolean => {
    const currentMap = form.getValues("sorenessMap") || {};
    return bodyParts.some(part => currentMap[part] > 0);
  };
  
  // Handle clicking on a muscle group from the list
  const handleMuscleGroupClick = (bodyParts: string[]) => {
    const currentMap = { ...form.getValues("sorenessMap") };
    
    // Check if any of the parts in the group are already selected
    const anySelected = bodyParts.some(part => currentMap[part] > 0);
    
    // Toggle as a group - if any are selected, deselect all; otherwise select all
    bodyParts.forEach(part => {
      if (anySelected) {
        delete currentMap[part]; // Remove if any are selected
      } else {
        currentMap[part] = 1; // Set initial soreness level
      }
    });
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-[#1a1d22] rounded-lg p-4">
                ...
                {/* Responsive container for muscle list and body diagram */}
                <div className="flex flex-col md:flex-row items-start gap-4">
                  {/* Muscle group list */}
                  <div className="w-full md:w-1/3 bg-gray-800 p-3 rounded-lg">
                    <h4 className="text-primary font-medium mb-2">
                      {bodyView === 'front' ? 'Front Muscle Groups' : 'Back Muscle Groups'}
                    </h4>
                    <div className="space-y-2">
                      {bodyView === 'front' ? (
                        ...
                        <>
                          <div 
                            className={`cursor-pointer p-2 rounded-md ${hasSelectedMuscle('left_shoulder', 'right_shoulder') ? 'bg-primary/20 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
                            onClick={() => handleMuscleGroupClick(['left_shoulder', 'right_shoulder'])}
                          >
                            Shoulders
                          </div>
                          <div 
                            className={`cursor-pointer p-2 rounded-md ${hasSelectedMuscle('chest') ? 'bg-primary/20 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
                            onClick={() => handleMuscleGroupClick(['chest'])}
                          >
                            Chest
                          </div>
                          <div 
                            className={`cursor-pointer p-2 rounded-md ${hasSelectedMuscle('abs') ? 'bg-primary/20 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
                            onClick={() => handleMuscleGroupClick(['abs'])}
                          >
                            Abs
                          </div>
                          <div 
                            className={`cursor-pointer p-2 rounded-md ${hasSelectedMuscle('left_quad', 'right_quad', 'left_shin', 'right_shin') ? 'bg-primary/20 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
                            onClick={() => handleMuscleGroupClick(['left_quad', 'right_quad', 'left_shin', 'right_shin'])}
                          >
                            Legs
                          </div>
                          <div 
                            className={`cursor-pointer p-2 rounded-md ${hasSelectedMuscle('neck') ? 'bg-primary/20 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
                            onClick={() => handleMuscleGroupClick(['neck'])}
                          >
                            Neck
                          </div>
                        </>
                        ...
                        <>
                          <div 
                            className={`cursor-pointer p-2 rounded-md ${hasSelectedMuscle('left_hamstring', 'right_hamstring') ? 'bg-primary/20 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
                            onClick={() => handleMuscleGroupClick(['left_hamstring', 'right_hamstring'])}
                          >
                            Hamstrings
                          </div>
                          <div 
                            className={`cursor-pointer p-2 rounded-md ${hasSelectedMuscle('upper_back', 'mid_back', 'lower_back') ? 'bg-primary/20 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
                            onClick={() => handleMuscleGroupClick(['upper_back', 'mid_back', 'lower_back'])}
                          >
                            Back
                          </div>
                          <div 
                            className={`cursor-pointer p-2 rounded-md ${hasSelectedMuscle('left_glute', 'right_glute') ? 'bg-primary/20 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
                            onClick={() => handleMuscleGroupClick(['left_glute', 'right_glute'])}
                          >
                            Glutes
                          </div>
                          <div 
                            className={`cursor-pointer p-2 rounded-md ${hasSelectedMuscle('left_calf', 'right_calf') ? 'bg-primary/20 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
                            onClick={() => handleMuscleGroupClick(['left_calf', 'right_calf'])}
                          >
                            Calves
                          </div>
                        </>
                  ...
                  {/* Body diagram */}
                  <div className="relative w-full md:w-2/3 flex justify-center items-center mt-4 md:mt-0">
  ...
  // Check if any of the specified muscles are selected
  function hasSelectedMuscle(...bodyParts: string[]): boolean {
    const currentMap = form.getValues("sorenessMap") || {};
    return bodyParts.some(part => currentMap[part] > 0);
  }
  
  // Handle clicking on a muscle group from the list
  function handleMuscleGroupClick(bodyParts: string[]) {
    const currentMap = { ...form.getValues("sorenessMap") };
